			...
			<fieldset>
				...
				<ul class="app_list">
					...
					<li class="clear">
						...
						<div class="app_content"><input type="password" name="password_chk" class="w100p" id="password_chk_lbl" placeholder="암호를 입력해주세요" required/></div>
					</li>
					<li class="clear">
			...
			$.ajax({
				...
				success:function(result) {
					if(result == 0) {
						...
						$("#msg").remove();
						$("#user_id_lbl").after("<div id='msg' style='color:blue'>사용가능한ID입니다</div>");
					}else{
						$("#btn_insert").attr("disabled",true);
						$("#btn_insert").css("opacity","0.5");
						$("#msg").remove();
						$("#user_id_lbl").after("<div id ='msg' style='color:green'>중복ID존재합니다</div>");
					}
				},
				error:function() {
					alert("RestAP서버가 작동하지 않습니다 ");
				}
			});
