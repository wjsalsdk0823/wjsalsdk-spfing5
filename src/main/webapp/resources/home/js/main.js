...
// PC용 서브메뉴 사라지는 액션
var isOver1 = false;//대메뉴 1이 false일때 서브메뉴가 사라지는 조건
var isOverSub1 = false;//서브메뉴가1번이 false일때 서브메뉴가 사라지느 ㄴ조건
//1번 서브메뉴 숨기는 함수(아래)
function goHide1() {
	if(!isOver1 && !isOverSub1) {//2개변수값이 false일때 구현 조건이 됩니다.
		$('.gnb_depth2_1').stop().fadeOut('fast');
	}
}
var isOver2 = false;//대메뉴2이 false 일때 서브메뉴가 사라지는 조건
var isOver22 = false;//서브메뉴가 2번이 false일때 서브메뉴가 사라지는 조건
//2번 서브메뉴 숨기는 함수(아래)
function goHide2() {
...
$(document).ready(function(){
	...
	$('.openAll1').mouseout(function(){
		isOver1 = false;
		setTimeout('goHide1()',200);//기본은 goHidel()이것만 하면되는데,mouseout이벤트 놓치면 안되기때문에,
	});
	//서브메뉴1 액션
	$('.gnb_depth2_1').mouseover(function(){
		isOverSub1 = true;//서브메뉴가 1 계속 나타납니다.
	});
	...
	$('.openAll2').mouseover(function(){
		if(parseInt($('header').css('width'))>=1049) {
			$('.gnb_depth2_2').stop().fadeIn('fast');//서브메뉴1번 나타나게 처리
		}
		isOver2 = true;
	});
	...
	$('.gnb_depth2_2').mouseover(function(){
		isOverSub2 = true;//서브메뉴가 2 계속 나타납니다.
	});
