 ...
 */
public class Step1 {
	public void Step1() {//클래스명과 같은 이름의 메서드를 생성자 메서드 자동
		//자동으로 만들어집니다.
		//중요한 이유는 다른 클래스에서  Step1을 객체로 만들 떄 생정자 메서드가 필요  
		//Step1 aaa =new Step1();
		//자바앱에서는 객체를 생성자 메서드 만들때 필수
		//단, 스프링에선@inject으로 객체를 만들어서 사용
	}
    //전역변수(멤버변수)는 Step1클래스에 영향을 모두 주는 변수
	...
	public static void main(String[] args) {
		...
		printMember(names,ages,phoneNums);//메서드 1번호출하면3명이 모두 출력
		//메서드명이 같습니다. 단  로드된 파라미터 타입,개수 다른 메서드를 호출=오버로드
	}
	...
	private static void printMember(String name, int age, String phoneNum) {
		// printMember(파라미터1, parme2, param3)
		System.out.println("입력하신 회원의 이름은 "+name+"님, 나이는  "+age+", 폰번호는 "+phoneNum);
		//println메서드는 출력 후 줄바꿈 명령실행 println=print + new line
	}
